...
func (r *FineTuneJobRepo) AddMetric(jobID string, name string, point domain.MetricPoint) error {
	r.store.mu.Lock()
	defer r.store.mu.Unlock()
	for _, j := range r.store.FineTuneJobs {
		if j.ID != jobID {
...
func (r *FineTuneJobRepo) SetError(jobID string, errMsg string) error {
	r.store.mu.Lock()
	defer r.store.mu.Unlock()
	for _, j := range r.store.FineTuneJobs {
		if j.ID == jobID {
