...
func (m *mockLLM) GenerateResponse(
	...
	return domain.Message{Role: "assistant", Content: "done"}, nil
}
...
func (m *mockTool) Execute(_ context.Context, _ domain.ToolInput) (domain.ToolOutput, error) {
	m.executed = true
	return domain.ToolOutput{Result: "executed"}, nil
}
...
func TestReflectAgent_Initialize(t *testing.T) {
	...
	config := domain.AgentConfig{Name: "test", MaxIterations: 5, Timeout: time.Minute}
	if err := a.Initialize(context.Background(), config); err != nil {
...
func TestReflectAgent_Execute_NoTools_Done(t *testing.T) {
	...
	_ = a.Initialize(context.Background(), domain.AgentConfig{MaxIterations: 5, Timeout: time.Minute})
